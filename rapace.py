...
import cv2                       # Pour la capture vidéo et traitement image
import threading                 # Pour créer des threads (IA en parallèle)
import time                      # Gestion du temps et des délais
import os                        # Gestion de fichiers et variables d'environnement
import tkinter as tk             # Interface graphique
from tkinter import scrolledtext # Zone texte défilante pour logs
from PIL import Image, ImageTk   # Pour convertir images OpenCV en affichage Tkinter
from ultralytics import YOLO     # Modèle YOLO pour détection IA
import numpy as np               # Calculs mathématiques (tableaux, matrices)
import pygame                    # Pour jouer les sons d’alerte
from datetime import datetime    # Pour dater les logs
import serial                    # Communication série avec l’ESP32/ELRS

# On enlève les logs OpenCV inutiles
os.environ['OPENCV_VIDEOIO_LOGLEVEL'] = '0'
os.environ['OPENCV_OPENCL_CACHE_CLEANUP'] = '0'

# Classe principale RAPACE
class RAPACE:
    def __init__(self, root):
        self.root = root
        self.root.title("RAPACE")
        self.root.configure(bg="#020c1b")  # Pour rendre plus joli l'appli

        # J'ai ajouté une icône à l'application
        try:
            self.root.iconbitmap("logo.ico")
        except:
            pass  # Si le fichier logo.ico est absent on continue

        # On configure l'ELRS
        try:
            self.ser = serial.Serial('COM3', 115200, timeout=0.1)  # Port série vers ESP32/ELRS
        except:
            self.ser = None
            print("ERREUR : Module ELRS non détecté sur COM3")

        # On charge la caméra et l'IA
        self.model = YOLO("best.pt")   # Chargement du modèle YOLO
        self.cap = cv2.VideoCapture(0) # Capture vidéo caméra interne (0)

        # Variables pour suivi IA
        self.frame = None
        self.running = True
        ...
        # On met les coordonnées des servos de 0 à 100%
        self.servo_x = 50
        self.servo_y = 50

        # Modes et options
        self.night_mode = False       # Vision nocturne
        self.auto_mode = False        # Mode automatique de suivi IA
        self.flip_view = True         # Inversion de l'image
        ...
        self.init_tracker_module()    # Tracker KCF pour suivi cible
        self.setup_ui()               # Création interface graphique

        # Lien souris pour pilotage manuel
    ...
    def setup_ui(self):
        ...
        # On met à gauche la vidéo
        self.left_panel = tk.Frame(self.main_container, bg="#020c1b")
        self.left_panel.pack(side="left")
        self.canvas = tk.Canvas(self.left_panel, width=640, height=480, bg="#000",
        ...
        tk.Label(self.side_panel, text="DONNÉES TACTIQUES", fg="#00f5d4",
                 bg="#020c1b", font=("Courier", 11, "bold")).pack(anchor="w")
        self.log_area = scrolledtext.ScrolledText(self.side_panel, width=35, height=15,
                                                  bg="#011627", fg="#4cc9f0", font=("Courier", 9), bd=0)
        self.log_area.pack(pady=(5, 15))

        # Boutons de contrôle
        tk.Label(self.side_panel, text="CONTRÔLE", fg="#00f5d4",
                 bg="#020c1b", font=("Courier", 11, "bold")).pack(anchor="w")
        ...
        self.status_bar = tk.Label(self.status_frame, text="OPÉRATIONNEL", fg="#fff",
                                   bg="#001d3d", font=("Courier", 12, "bold"))
        self.status_bar.pack(pady=5)

    def create_button(self, text, command):
        btn = tk.Button(self.side_panel, text=text, command=command, bg="#1a2e44",
                        fg="#ffffff", font=("Courier", 10), relief="flat", width=28, pady=8)
        btn.pack(pady=4)
        return btn
    ...
    def toggle_nv(self):
        ...
        self.btn_nv.config(text=f"VISION NOCTURNE [{'ON' if self.night_mode else 'OFF'}]",
                           bg="#004b23" if self.night_mode else "#1a2e44")
    ...
    def toggle_mode(self):
        ...
        self.btn_mode.config(text=f"MODE : {'AUTO' if self.auto_mode else 'MANUEL'}",
                             bg="#9b2226" if self.auto_mode else "#1a2e44")
    ...
    def mouse_move(self, event):
        if not self.auto_mode:
            ...
            self.send_to_elrs()  # Envoi au drone via ESP32
    ...
    def ia_detection_loop(self):
        while self.running:
            if self.frame is not None:
                # Détection YOLO sur la frame actuelle
                ...
                if len(results[0].boxes) > 0:
                    ...
                    # Initialisation tracker si pas déjà en suivi
    ...
    def update_view(self):
        ...
        self.frame = frame.copy()   # Stockage pour IA
        display_frame = frame.copy() # Frame pour affichage

        # Filtre vision nocturne
        if self.night_mode:
            display_frame[:,:,0] = 0
            display_frame[:,:,2] = 0
            display_frame = cv2.addWeighted(display_frame, 1.4, display_frame, 0, 15)

        # Tracker et verrouillage cible
        if self.is_tracking:
            ...
            if success:
                ...
                # Mode automatique : ajustement servos
                if self.auto_mode:
                    diff_x = (cx - 320) / 320
                    diff_y = (cy - 240) / 240
                    self.servo_x = np.clip(self.servo_x + (diff_x * 5), 0, 100)
                    self.servo_y = np.clip(self.servo_y + (diff_y * 5), 0, 100)
                    self.send_to_elrs()  # Envoi coordonnées au drone

                # Gestion verrouillage cible
                if abs(cx - 320) < 100 and abs(cy - 240) < 100:
                    if not self.sound_playing:
                        if self.alarm_sound: self.alarm_sound.play(-1)  # Boucle son alarme
        ...
        self.root.after(10, self.update_view)  # Boucle affichage continue
